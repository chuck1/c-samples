...
void	Calculations()
{
	...
	shadowMatrix(floorShadow, floor_.plane_, light_.camera_.eye_);
...
void	DrawFloorTop()
{
	...
	if (renderShadow && stencilShadow) PrepStencilShadow();
...
void liz::redraw()
{
	...
	glPushMatrix();
	{
		...
		if(renderReflection) RenderReflection();

		DrawFloorBottom();
		DrawFloorTop();
		
		if (renderDinosaur) drawDinosaur();// Draw "actual" dinosaur, not its reflection.
		
		if (renderShadow) RenderShaodw();
		
		DrawLightVisualtization();
	}
	glPopMatrix();
